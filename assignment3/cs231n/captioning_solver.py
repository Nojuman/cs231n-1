...
from cs231n.coco_utils import sample_coco_minibatch, get_all_data
...
class CaptioningSolver(object):
  ...
  - model.sample(features) must be a function that generates test-time captions
  ...
  def __init__(self, model, data, **kwargs):
    ...
    # Unpack data
    self.train_captions, self.train_features, _ = get_all_data(self.data, split='train')
    self.val_captions, self.val_features, _ = get_all_data(self.data, split='val')
  ...
  def check_accuracy(self, X, y, num_samples=None, batch_size=100):
    """
    Check accuracy of the captioning model using BLEU.

    Inputs:
    - X: Array of image features, of shape (N, d_1, ..., d_k)
    - y: Array of caption sequences, of shape (N,T)
    ...
    Returns:
    - acc: Scalar giving the BLEU score.
    """
    ...
    for i in xrange(num_batches):
      ...
      captions = self.model.sample(X[start:end], max_length=y.shape[1])
      y_pred.append(captions)
    y_pred = np.hstack(y_pred)

    # TODO use BLEU. Using slow Jaccard for now.
    scores = []
    for tru_caption, obs_caption in itertools.izip(y, y_pred):
        t = set(tru_caption)
        o = set(obs_caption)
        scores.append(float(len(t & o)) / float(len(t | o)))
    acc = np.mean(scores)
  ...
  def train(self):
    ...
    for t in xrange(num_iterations):
      ...
      # At the end of every epoch, increment the epoch counter, decay the
      # learning rate, and compute training + validation accuracy
      epoch_end = (t + 1) % iterations_per_epoch == 0
      if epoch_end:
        self.epoch += 1
        for k in self.optim_configs:
          self.optim_configs[k]['learning_rate'] *= self.lr_decay
        train_acc = self.check_accuracy(self.train_features, self.train_captions, num_samples=100)
        val_acc = self.check_accuracy(self.val_features, self.val_captions, num_samples=100)
